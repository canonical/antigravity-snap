...
- **Pull Requests**: For human PRs, runs matrix build and smoke tests across `amd64` and `arm64`. For automated release monitor PRs (`automation/update-antigravity-*`), runs fast security & author validation and automatically squash-merges into `main`, avoiding duplicate builds.
- **Pushes to `main`**: Runs matrix build and smoke tests across both architectures once, then passes verified artifacts to `publish-edge` to release to `latest/edge` after human environment approval.
...
- `amd64` builds run on `ubuntu-24.04` (or `ubuntu-26.04`)
- `arm64` builds run on `ubuntu-24.04-arm` (or `ubuntu-26.04-arm`)
...
|---|---|---|---|
| `pull_request` (bot) | `automerge` | N/A | Fast author/commit validation (~10s) and automatic squash-merge into `main`. Heavy matrix builds are skipped in the PR to save runner minutes. |
| `pull_request` (human) | `build` | N/A | PR validation and smoke testing across `amd64` and `arm64`. No store publishing. |
| `push` to `main` | `build` &rarr; `publish-edge` &rarr; `release` | `latest/edge` | Single matrix build & smoke test across architectures, awaits human environment approval, publishes to `latest/edge` via `snapcore/action-publish`, and creates GitHub tag and release. |
...
2. Validates that only `snap/snapcraft.yaml` was modified in the PR.
3. Verifies that all commits in the PR were authored exclusively by `github-actions[bot]`.
4. Uses GitHub CLI to automatically squash-merge the pull request into `main` and delete the automation branch (`gh pr merge --squash --delete-branch`). The full matrix build is then executed once on `main`.
...
- `canonical/has-signed-canonical-cla@19bae73390fdbfdc1ef9a9bb9408d87a1de755f6` (`v2`)
